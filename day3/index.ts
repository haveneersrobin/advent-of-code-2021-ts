...
const processAndTransposeInput = (input: string) => {
  const arrayOfStrings = input.split('\n').map(str => str.trim());
  const zeroBitCount = new Array<number>(arrayOfStrings[0].length - 1).fill(0);
  const oneBitCount = new Array<number>(arrayOfStrings[0].length - 1).fill(0);

  arrayOfStrings.map(string => {
    string.split('').map((bit: string, column: number) => {
      bit === '0'
...
const getGammaRate = (zeroBitCount: number[], oneBitCount: number[]) => {
  let resultString = '';
  zeroBitCount.forEach((el, index) => (resultString += el > oneBitCount[index] ? '0' : '1'));
  return resultString;
};

const invert = (input: string) =>
  input
    .split('')
    .map(el => (el === '0' ? '1' : '0'))
    .join('');
const toDecimal = (binaryInput: string) => parseInt(binaryInput, 2);

const calculatePrintResult = (input: string) => {
